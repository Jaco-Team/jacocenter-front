...
созданного клиента. Обычное сохранение требует имя; если клиент не сообщил имя,
оператор может явно выбрать «Создать без имени», и в Chef будет записано
`Не указал`. Автоматическое создание записи с именем `Клиент` удалено, чтобы не
загрязнять Chef placeholder-пользователями.
