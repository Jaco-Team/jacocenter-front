...
import type { CityDto, PointDto, SlotsDto, StreetDto, ValidationDto, ZoneDto } from './deliveryMapper';
...
export const deliveryApi = {
  ...
  async zones(cityId: number) { const response = await apiRequest<{ data: { zones?: ZoneDto[] } }>(`/delivery/zones${queryString({ city_id: cityId })}`); return (response.data.zones ?? []).map(mapZone); },
