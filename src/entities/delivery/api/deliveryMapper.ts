...
export const mapPoint = (value: PointDto): Point => ({ id: Number(value.id), cityId: Number(value.city_id), name: String((value.address || value.name || '').trim()), base: String(value.base ?? ''), address: String(value.address ?? ''), latitude: value.latitude == null ? null : Number(value.latitude), longitude: value.longitude == null ? null : Number(value.longitude) });
