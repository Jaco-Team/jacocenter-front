...
import type { PromoCheckDto, PromoDto, PromoEvaluationDto } from './promoMapper';
...
export const promoApi = {
  async list(cityId: number) { const response = await apiRequest<{ data: { items?: PromoDto[] } }>(`/promos${queryString({ city_id: cityId })}`); return (response.data.items ?? []).map(mapPromo); },
  async check(code: string, cityId: number) { const response = await apiRequest<{ data: PromoCheckDto }>('/promos/check', { method: 'POST', body: { code, city_id: cityId } }); return mapCheck(response.data); },
  async evaluate(input: PromoEvaluateInput) {
    const response = await apiRequest<{ data: PromoEvaluationDto }>('/promos/evaluate', { method: 'POST', body: { code: input.code, city_id: input.cityId, type_order: input.typeOrder, items: cart(input.items), ...(input.pointId === undefined ? {} : { point_id: input.pointId }), ...(input.customerId === undefined ? {} : { customer_id: input.customerId }), ...(input.phone ? { phone: input.phone } : {}), ...(input.weekday === undefined ? {} : { weekday: input.weekday }), ...(input.time ? { time: input.time } : {}) } });
